...
+
Sie können *Spalten ein-/ausblenden* verwenden, um Spalten in der Tabelle hinzuzufügen oder zu entfernen. Standardmäßig zeigt die Tabelle Ihre zuvor gespeicherten Spalteneinstellungen an. Neu hinzugefügte Spalten, wie die Spalten *Abteilung* oder *Gesamt-Fußabdruck*, sind standardmäßig ausgeblendet und müssen manuell ausgewählt werden, um in der Tabelle angezeigt zu werden. Sie können beliebige Spalten auswählen oder abwählen, und Ihre Einstellungen werden für die spätere Verwendung gespeichert. Beim Herunterladen des CSV-Berichts werden alle verfügbaren Spalten unabhängig von Ihren Anzeigeeinstellungen exportiert.
