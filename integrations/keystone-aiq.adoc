...
* *Service Level Agreement*: Dieser Status wird angezeigt, wenn mindestens eines Ihrer Abonnements einen gültigen Service Level Agreement (SLA) für Nachhaltigkeit mit NetApp hat. Der Status gibt an, ob die Abonnements die in Ihrem SLA definierten Nachhaltigkeitskriterien erfüllen. Wenn Ihre Abonnements die SLA-Kriterien erfüllen, können Sie den Status als _Alle OK_ sehen.
