...
NOTE: Der Kapazitätsverbrauch in Keystone-Abonnements wird in TiBs auf den Dashboards und Berichten angezeigt und auf zwei Dezimalstellen gerundet. Bei einem Verbrauch von weniger als 0,01 TiB wird der Wert als 0 oder No Usage angezeigt. Die Daten auf diesen Bildschirmen werden in UTC-Zeit (Serverzeitzone) angezeigt. Wenn Sie ein Datum für eine Abfrage eingeben, wird dieses automatisch als UTC-Zeit betrachtet. Weitere Informationen zu den Nutzungsmetriken finden Sie unter link:../concepts/metrics.html#metrics-measurement["Kennzahlenmessung"]. Informationen zu den verschiedenen Kapazitäten, die in Keystone verwendet werden, finden Sie unter link:../concepts/supported-storage-capacity.html["Speicherkapazitätstypen in Keystone"].
