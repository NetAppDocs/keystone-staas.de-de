...
NOTE: Sie können die *Prognose*-Funktion nutzen, um die voraussichtliche Kapazitätsauslastung in der *Historischen Verbrauchsansicht* und den voraussichtlichen aufgelaufenen Burst in der *Aufgelaufenen Burst*-Ansicht anzuzeigen. Weitere Informationen finden Sie unter link:../integrations/consumption-tab.html#view-forecast-data["Prognosedaten anzeigen"].
...
+
image:consumption-trend-burst.png["Screenshot mit der Anzeige des historischen Verbrauchs"]

+
Das Liniendiagramm zeigt historische Verbrauchsdaten und ermöglicht es Nutzern, Trends über einen ausgewählten Zeitraum zu analysieren. Das Diagramm zeigt die folgenden Kennzahlen:

+
** *Ausgenutzte Kapazität*: Gesamtspeicherplatz in Verwendung, einschließlich sowohl Daten- als auch Snapshot-Kapazität.
** *Daten*: Speicherkapazität, die von Benutzerdaten belegt wird.
** *Snapshot*: Speicherkapazität, die von Snapshot-Daten belegt wird.
** *Verpflichtet*: Die vertraglich zugesicherte Kapazität.
** *Burst-Kapazität*: Das maximal zulässige Burst-Limit für Ihr Abonnement.
** *Über der Burst-Kapazität*: Speichernutzung über dem Burst-Limit. Diese Metrik wird nur angezeigt, wenn das Burst-Limit überschritten wird.
+
Basierend auf dem Datumsbereich Ihrer Abfrage zeigen die Nutzungsdiagramme bis zu 30 Datenerfassungspunkte an. Sie können den Mauszeiger über das Diagramm bewegen, um die Nutzungsaufschlüsselung an jedem Datenerfassungspunkt anzuzeigen.
...
Sie können die aufgelaufene Burst-Datennutzung über die Console oder Digital Advisor anzeigen. Wenn Sie im Tab *Consumption trend* in der Console im Dropdown-Menü *View* die Option *Accrued burst* oder im Tab *Consumption Trend* in Digital Advisor die Option *Invoiced Accrued Burst* ausgewählt haben, können Sie die aufgelaufene Burst-Datennutzung monatlich oder vierteljährlich anzeigen, abhängig von Ihrem gewählten Abrechnungszeitraum.

Wenn die *Prognose*-Funktion in der Konsole deaktiviert ist, können Sie die abgerechnete Burst-Nutzung sowie die nicht abgerechnete Burst-Nutzung (aufgelaufen, aber noch nicht in Rechnung gestellt) und die vorläufige Burst-Nutzung (im aktuellen Abrechnungszeitraum aufgelaufen und noch nicht in Rechnung gestellt) einsehen. Wenn die *Prognose*-Funktion aktiviert ist, zeigt das Diagramm zusätzlich die prognostizierte aufgelaufene Burst-Nutzung für die nächsten 120 Tage ab dem aktuellen Datum an. Weitere Informationen zur *Prognose*-Funktion finden Sie unter link:../integrations/consumption-tab.html#view-forecast-data["Prognosedaten anzeigen"].
...
In der Konsole können Sie im Tab *Verbrauchstrend* den *Prognose*-Schalter verwenden, um die prognostizierte Kapazitätsnutzung in der Ansicht *Historischer Verbrauch* und den prognostizierten aufgelaufenen Burst in der Ansicht *Aufgelaufener Burst* anzuzeigen. Der Schalter ist in beiden Ansichten standardmäßig aktiviert.

In der Ansicht „Historischer Verbrauch“ werden die Verbrauchsdaten der letzten 30 Tage sowie eine 120-Tage-Prognose ab dem aktuellen Datum im selben Diagramm dargestellt. Insgesamt werden bis zu 30 Datenpunkte abgebildet (20 für historische Daten und 10 für Prognosedaten), mit einer vertikalen Linie, die die beiden Segmente voneinander trennt.

image:forecast-1.png["Screenshot mit Prognosedaten"]

In der Ansicht *Aufgelaufenes Burst-Kontingent* zeigt das Diagramm die letzten 6 Monate der aufgelaufenen Burst-Historie und eine 120-Tage-Prognose ab dem aktuellen Datum, wobei eine vertikale Linie die historischen und prognostizierten Daten trennt. Der Balken für den aktuellen Abrechnungszeitraum zeigt sowohl die vorläufige Burst-Nutzung als auch die Prognosedaten für die verbleibenden Tage an. Sie können auf jeden Balken klicken, um die tagesgenaue Aufschlüsselung in der Tabelle *Aufgelaufenes Burst-Kontingent nach Tagen* anzuzeigen. Weitere Informationen zur Tabelle *Aufgelaufenes Burst-Kontingent nach Tagen* finden Sie unter link:../integrations/consumption-tab.html#view-daily-accrued-burst-data-usage["Anzeige der täglichen angesammelten Burst-Datennutzung"].

image:accrued-burst-forecast.png["Screenshot mit der kumulierten Ausbruchsprognose"]

Wenn Sie einen Datumsbereich auswählen, der nicht mit dem aktuellen Datum endet, wird die Prognose automatisch deaktiviert. Um sie wieder zu aktivieren, werden Sie in einem Bestätigungsdialog darauf hingewiesen, dass Prognosedaten nur verfügbar sind, wenn der ausgewählte Zeitraum mit dem aktuellen Datum endet. Wenn Sie den Dialog bestätigen, wird die Ansicht auf den Standardbereich für die ausgewählte Ansicht zurückgesetzt: die letzten 30 Tage der historischen Daten plus die nächsten 120 Tage Prognose in der Ansicht *Historischer Verbrauch* oder die letzten 6 Monate der aufgelaufenen Burst-Historie plus die nächsten 120 Tage Prognose in der Ansicht *Accrued burst*.

Wenn der prognostizierte Verbrauch die zugesicherte Kapazität voraussichtlich übersteigt, erscheint im Diagramm eine orangefarbene Warnanzeige, die eine Kapazitätserweiterung empfiehlt. Wenn der prognostizierte Verbrauch die Burst-Kapazität voraussichtlich übersteigt, erscheint ebenfalls eine rote Warnanzeige, die eine Kapazitätserweiterung empfiehlt.
...
NOTE: Die im Diagramm dargestellten Daten hängen davon ab, ob die *Vorhersage*-Funktion aktiviert oder deaktiviert ist.

image:bxp-accrued-burst-days-1.png["Screenshot der Tabelle mit den aufgelaufenen Bursts nach Tagen"]

Sie können die verfügbaren Daten auch im Excel-Format herunterladen, einschließlich des Burst-Status (fakturiert, nicht fakturiert, vorläufig oder prognostiziert, je nach Anwendbarkeit) und des Abrechnungszeitraums.
