...
Für jedes Abonnement können Sie die folgenden Informationen sehen:
...
* *Enddatum*: Das Enddatum des Abonnements. Ein leeres Datumsfeld zeigt an, dass das Abonnement kein Enddatum hat und automatisch erneuert wird. Basierend auf diesem Datum werden möglicherweise Benachrichtigungen für Abonnements angezeigt, die kurz vor dem Ende stehen oder an die Richtlinien zur automatischen Verlängerung angehängt sind.
...
Sie sehen eine tägliche Aufschlüsselung der gemessenen Nachhaltigkeitsmetriken für den aktuellen Monat:

image:sla-sustainability.png["sla-Details, die Details zur Nachhaltigkeit enthalten"]

Die folgenden Details werden angezeigt:

* *Abonnement*: Die Abonnementnummer, sowie das Startdatum, das Enddatum und die Abrechnungsfrist des von Ihnen gewählten Abonnements.
...
* *Tag*: Der Tag des Monats, an dem dieses Datenelement abgerufen wird.
* *Generated Date*: Das tatsächliche Datum des Monats.
...
Wenn es zu einer SLA-Verletzung kommt, wird ein Symbol angezeigt image:warning.png["Symbol für Warnung"] Neben der Säule informiert Sie über die Art der Verletzung. Die folgenden Warnungen werden angezeigt:
...
* Abonnementdetails: Das Start- und Enddatum des Abonnements sowie der Abrechnungszeitraum, z. B. vierteljährlich, jährlich usw.
* Nutzungsdiagramme: Die Balkendiagramme zeigen den Namen des Service-Levels und die Kapazität an, die für diesen Service-Level für den Datumsbereich verbraucht wurde. Das Datum und die Uhrzeit der Sammlung werden unten im Diagramm angezeigt.
+
...
[%collapsible]
====
Wenn Sie den erweiterten Datenschutz-Zusatzdienst abonniert haben, können Sie die Aufschlüsselungsdaten für die MetroCluster-Partnerseiten auf der Registerkarte *Zeitreihen* anzeigen.
...
Das folgende Bild zeigt die Diagramme für den Service-Level _Extreme_ (Basis-Service-Level) und eine primäre Abonnementnummer. Das Diagramm der gleichen Zeitreihen zeigt auch den Verbrauch der Spiegelstelle in einem helleren Farbton mit demselben Farbcode an, der für den primären Standort verwendet wird. Mit der Kurzinformation beim Mauszeiger wird der Aufschlüsselungsverbrauch (in tib) für die primären und gespiegelten Standorte, 1.02 tib bzw. 1.05 tib angezeigt.
