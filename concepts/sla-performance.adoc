...
[role="lead"]
Keystone STaaS bietet ein latenzbasiertes Leistungsziel (Service Level Objective, SLO), das die Latenz im 90. Perzentil pro Performance Service Level Instance (PSLI) für einheitlichen (ausgenommen das _Value_ Leistungsziel), blockoptimierten und AFX-Speicher anstrebt. Erfahren Sie, wie Sie die Leistung überprüfen, welche Regeln gelten und wie Sie Servicegutschriften erhalten, falls Keystone das Latenzziel für die genutzte Kapazität bis zum Burst-Limit nicht erreicht, wie in den Keystone-Bestellbedingungen festgelegt.
...
** Bei Verwendung des erweiterten Datensicherungs-Add-ons umfasst die Ziellatenz nur E/A-Operationen zum und vom lokalen Speichersystem.
...
* Gilt nur für NetApp AFF und AFX-Plattformen.
* Alle in der Keystone-Bestellung festgelegten Mindest- und Höchstkapazitäten sowie sonstige Beschränkungen pro Instanz des Performance-Service-Levels sind anwendbar.
* Alle Instanzen des Performance-Service-Levels erfordern die vollständige und kontinuierliche Aktivierung des Monitoring-Tools und AutoSupport (ASUP).
...
Das folgende Beispiel zeigt, wie Dienstgutschriften berechnet werden:
...
Basierend auf der Berechnung beträgt die Servicegutschrift 12 US-Dollar.
