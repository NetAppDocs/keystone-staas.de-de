...
---
= Daten-Tiering-Zusatzdienst für Keystone
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Keystone STaaS-Standardservices für Datei- und Block-Storage umfassen Tiering-Funktionen, die seltener verwendete Daten identifizieren und diese in von Keystone STaaS unterstützten NetApp Cold Storage verschieben. Sie können Daten-Tiering als Add-on-Service verwenden, wenn Sie Ihre kalten Daten in beliebigen von Keystone STaaS unterstützten Storage anderer Anbieter verschieben möchten. Dieser Add-on-Service wird nur beim einheitlichen Datenspeicher-Typ unterstützt.

unter link:../concepts/service-levels.html["Leistungsservicelevel in Keystone"] .


NOTE: Der Tiering-Add-on-Service ist nur erforderlich, wenn Daten auf Storage anderer Anbieter wie Amazon Web Services (AWS) S3, Azure Blob, Google Cloud Platform (GCP) und anderen von Keystone STaaS unterstützten, S3-kompatiblen Object Storage von Drittanbietern ausgelagert werden. Dieser Service ist nur für den einheitlichen Datenspeichertyp verfügbar.
